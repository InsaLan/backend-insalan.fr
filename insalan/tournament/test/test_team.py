...
class TournamentTeamEndpoints(TestCase):
    ...
    def test_expand_threshold_when_enough_players_pay(self) -> None:
        """When enough teams have paid players, tournament threshold expands."""
        # Create a small tournament with low thresholds
        event = Event.objects.create(
        ...
        game = Game.objects.create(name="Tiny Game", substitute_players_per_team=0)

        tr = EventTournament.objects.create(
            game=game,
            event=event,
            is_announced=True,
            max_team_thresholds=[1,2,4],
        )

        # Create two teams with one player each
        u1 = User.objects.create_user(username="t1", password="pw1", email="test1@test.fr")
        u2 = User.objects.create_user(username="t2", password="pw2", email="test2@test.fr")

        team1 = Team.objects.create(name="T1", tournament=tr, password=make_password("p"))
        team2 = Team.objects.create(name="T2", tournament=tr, password=make_password("p"))

        p1 = Player.objects.create(user=u1, team=team1, name_in_game="p1")
        p2 = Player.objects.create(user=u2, team=team2, name_in_game="p2")

        # Initially only 0 validated, current max is 1 (index 0)
        self.assertEqual(tr.current_threshold_index, 0)
        self.assertEqual(tr.get_max_team(), 1)

        # Paying first player validates first team but should NOT expand yet
        p1.payment_status = PaymentStatus.PAID
        p1.save()

        team1.refresh_from_db()
        tr.refresh_from_db()
        self.assertTrue(Team.objects.get(id=team1.id).validated)
        self.assertEqual(tr.current_threshold_index, 0)

        # Paying second player should trigger expansion to next threshold (2)
        p2.payment_status = PaymentStatus.PAID
        p2.save()

        tr.refresh_from_db()
        team2.refresh_from_db()

        # tournament should have expanded and both teams validated
        self.assertEqual(tr.current_threshold_index, 1)
        self.assertEqual(tr.get_max_team(), 2)
        self.assertTrue(Team.objects.get(id=team2.id).validated)

    def test_no_expand_when_not_enough_potential(self) -> None:
        """Threshold should not expand if validated+potential < next threshold."""
        event = Event.objects.create(
            name="NoExpand Event",
            date_start=date(2024,2,1),
            date_end=date(2024,2,2),
            description="",
        )
        game = Game.objects.create(name="Bigger Game", substitute_players_per_team=0)

        # Set next threshold high so expansion is impossible with only two teams
        tr = EventTournament.objects.create(
            game=game,
            event=event,
            is_announced=True,
            max_team_thresholds=[1,10],
        )

        u1 = User.objects.create_user(username="nx1", password="pw1", email="test1@test.fr")
        u2 = User.objects.create_user(username="nx2", password="pw2", email="test2@test.fr")

        team1 = Team.objects.create(name="NX1", tournament=tr, password=make_password("p"))
        team2 = Team.objects.create(name="NX2", tournament=tr, password=make_password("p"))
        ...
        # Refresh and check: only one team validated and threshold should NOT expand
        tr.refresh_from_db()
