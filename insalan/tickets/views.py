...
def get(request: HttpRequest, username: str, token: str) -> JsonResponse:
    ...
    try:
        ...
        return JsonResponse({"err": "Invalid uuid"}, status=status.HTTP_400_BAD_REQUEST)

    try:
        user = User.objects.get(username=username)
    except User.DoesNotExist:
        return JsonResponse({"err": "User not found"}, status=status.HTTP_404_NOT_FOUND)

    try:
        ticket = Ticket.objects.get(token=uuid.UUID(token), user=user)
    except Ticket.DoesNotExist:
        return JsonResponse(
            {"err": "Ticket not found"}, status=status.HTTP_404_NOT_FOUND
        )
